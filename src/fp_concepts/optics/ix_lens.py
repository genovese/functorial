...
from .profunctors import Forget, IndexedForget, Indexed, Star
...
def ilens[I, S, T, A, B](
    ...
    def the_lens(p):
        if isinstance(p, IndexedForget):
            # Reading path: extract index and focus, pass to r-producing function.
            # Never calls setter.
            f = IndexedForget.run(p)
            def read(i_acc, s):
                ix, a = get_with_index(s)
                return f(_pack_index(i_acc, ix), a)
            return IndexedForget(read, p._monoid)
        ...
        # Plain profunctor (Forget, Star, etc.): act as a plain lens, forgetting
        # the index.  Enables cast_as(LENS) and use with view, over, etc.
...
#
# An indexed lens whose focus IS the whole structure, and whose index
# IS the structure itself.  Primarily useful as a prefix in composition:
#
...
def _self_index_fn(p):
    if isinstance(p, IndexedForget):
        ...
        return IndexedForget(
            lambda i_acc, s: f(_pack_index(i_acc, s), s),
            p._monoid,
        )
    if isinstance(p, Indexed):
        f = Indexed.run(p)
...
def iview(optic) -> Function:
    """Returns the index and focus of an IxGetter or IxLens as a function s -> (i, a).

    iview :: IxGetter i s a -> s -> (i, a)

    """
    p = IndexedForget(lambda i, a: (i, a))
    result = IndexedForget.run(optic(p))   # i_acc -> s -> (i, a)
    return Function(lambda s: result(_MISSING, s))


def iover(optic, f: Callable) -> Function:
    """Modifies the focus with an indexed function, returning a function s -> t.

    iover :: IxLens i s t a b -> (i -> a -> b) -> s -> t

    """
    p = Indexed(f)
    result = Indexed.run(optic(p))         # i_acc -> s -> t
    return Function(lambda s: result(_MISSING, s))


def iput(optic, b) -> Function:
    """Replaces the focus with a constant value, returning a function s -> t.
