...
def traversable_examples():
    ...
    return [
        (str(State.eval(traverse(effn(State, f), t), 0).to_sexp()),
         '[102, [104, [106], [108], [110]], [112, [114, [116, [118], [120]]]]]',
         None),
        (State.exec(traverse(effn(State, f), t), 0), 10, None),
        (str(State.eval(itraverse(effn(State, g), t), 0).to_sexp()),
         '[([], 102), [([0], 104), [([0, 0], 106)], [([0, 1], 108)], [([0, 2], 110)]], [([1], 112), [([1, 0], 114), [([1, 0, 0], 116), [([1, 0, 0, 0], 118)], [([1, 0, 0, 1], 120)]]]]]',
         None),
        (State.exec(itraverse(effn(State,g), t), 0), 10, None),
        (State.run(traverse(effn(State, f), List.of(1, 2, 3, 4)), 0),
         ([102, 104, 106, 108], 4), None),
        (State.run(itraverse(effn(State, g), List.of(1, 2, 3, 4)), 0),
         ([(0, 102), (1, 104), (2, 106), (3, 108)], 4),
         None),
        (State.run(traverse(effn(State, f), d), 0),
         ({'a': 108, 'b': 120, 'c': 300}, 3), None),
        (State.run(itraverse(effn(State, g), d), 0),
